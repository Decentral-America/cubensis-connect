          ...
          {(props) => (
            ...
            >
              <button className={styles.copyButton} {...props}>
                <svg aria-hidden="true" className={styles.copyIcon} viewBox="0 0 14 14">
        ...
        <Tooltip content={t('address.edit')}>
          {(props) => (
            <button
              ...
              {...props}
            >
              <svg aria-hidden="true" className={styles.editIcon} viewBox="0 0 14 14">
        ...
        <Tooltip content={t('address.delete')}>
          {(props) => (
            <button
              ...
              {...props}
            >
              <svg aria-hidden="true" className={styles.deleteIcon} viewBox="0 0 18 18">
  ...
  const addressList = useMemo(
    () =>
      Object.entries(addresses)
        .filter(([address, name]) => icontains(address, search) || icontains(name, search))
    ...
    <div className={styles.content}>
      ...
      {Object.keys(addresses).length === 0 ? (
        <div className={styles.empty}>
          <p className={`headline2Bold ${styles.emptyTitle}`}>{t('address.emptyTitle')}</p>
          <p className={`basic500 ${styles.emptyDescription}`}>{t('address.emptyDescription')}</p>
          <Button view="submit" className={styles.addButton} onClick={addHandler}>
            {t('address.addButton')}
          </Button>
        </div>
      ) : (
