...
class InfoAdapter extends (CustomAdapter as any) {
  _code: number;
  [key: string]: any;
  ...
  getSignVersions() {
    return (CubensisConnectAdapter as any)._txVersion;
  }
}
...
export class MessageController extends EventEmitter {
  ...
  store: any;
  signTx: any;
  signDecentralChain: any;
  auth: any;
  signRequest: any;
  signBytes: any;
  broadcast: any;
  getMatcherPublicKey: any;
  getMessagesConfig: any;
  getPackConfig: any;
  assetInfo: any;
  ...
  constructor(options: any = {}) {
    ...
    this.assetInfo = options.assetInfoController.assetInfo.bind(options.assetInfoController);
  ...
  approve(id, account) {
    ...
    if (!message.account)
      return Promise.reject('Message has empty account filed and no address is provided');
  ...
  async _getMessageDataHash(data, account) {
    ...
    const [id, bytes] = await Promise.all([signable.getId(), signable.getBytes()]);
  ...
  async _validateAndTransform(message) {
    ...
    switch (message.type) {
      ...
      case 'transaction':
        ...
        result.bytes = Array.from((meta as any).bytes);
        result.lease = lease;
        result.json = json;
        break;
      // @ts-expect-error intentional fallthrough from cancelOrder to request
      case 'cancelOrder':
    ...
    switch (tx.type) {
      ...
      case 11:
        tx.data.transfers.forEach(({ amount }) => {
          if (!this._isMoneyLikeValuePositive(amount) && !this._isNumberLikePositive(amount)) {
            throw new Error('amount is not valid');
          }
        });
        break;
