...
export class RemoteConfigController {
  ...
  shouldIgnoreError(context, message) {
    ...
    if (ignoreErrorsConfig.ignoreAll) return true;

    const patterns = ignoreErrorsConfig[context];
    if (!Array.isArray(patterns)) return false;

    return patterns.some((str) => {
      if (typeof str !== 'string' || str.length > 200) return false;
      try {
        const re = new RegExp(str);
        // Guard against ReDoS: test with a timeout-like approach —
        // since JS has no regex timeout, at least reject overly complex patterns
