...
// config.json is a local-only file with optional API keys (not committed)
const statisticsApiKey = null;
...
export class StatisticsController {
  events: any[] = [];
  sended: Promise<any> = Promise.resolve();
  _idle: any = 0;
  controllers: any;
  store: any;
  version: string;
  ...
  constructor(store: any = {}, controllers: any) {
  ...
  addEventOnce(eventType: string, ms?: number, storeKey?: string) {
    ...
    const dateLast = state[storeKey] ? new Date(state[storeKey]) : new Date(dateNow.getTime() - ms);

    if (dateNow.getTime() - dateLast.getTime() >= ms) {
      const partial = {};
