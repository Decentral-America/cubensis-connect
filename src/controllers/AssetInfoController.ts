...
export class AssetInfoController {
  suspiciousAssets: string[] | undefined;
  suspiciousLastUpdated: number;
  getNode: any;
  getNetwork: any;
  store: any;
  ...
  isMaxAgeExceeded(lastUpdated: any) {
  ...
  async updateSuspiciousAssets() {
    ...
    if (
      ...
      (network === 'mainnet' &&
        Date.now() - new Date(this.suspiciousLastUpdated).getTime() > MAX_AGE)
    ) {
