...
/**
 * Validates an origin string to prevent prototype pollution and injection.
 * Must be a valid URL origin (protocol + host) or a recognized scheme.
 */
function isValidOrigin(origin: string | null | undefined): boolean {
  if (!origin || typeof origin !== 'string') return false;
  if (origin.length > 2048) return false;
  // Reject prototype pollution keys
  if (origin === '__proto__' || origin === 'constructor' || origin === 'prototype') return false;
  try {
    const url = new URL(origin);
    return !!url.protocol && !!url.host;
  } catch {
    // Also accept chrome-extension:// origins which may not parse with URL
