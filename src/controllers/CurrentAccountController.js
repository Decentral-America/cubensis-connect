...
export class CurrentAccountController {
  ...
  getByUrl(url) {
    ...
    if (url.match(/^(assets|transactions)/)) accept += 'large-significand-format=string';
                ...
                txHistory[0]
                  .reduce(
                    (ids, tx) => [
                      ...
                      ...(tx.stateChanges ? tx.stateChanges.transfers.map((x) => x.asset) : []),
                    ],
                    [],
                  )
                  .filter(
                    (assetId) => !!assetId && !assetExists(assetId) && isMaxAgeExceeded(assetId),
                  ),
              );
