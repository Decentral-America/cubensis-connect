...
Sentry.init({
  ...
  beforeSend: async (event, hint) => {
    ...
    const shouldIgnore = backgroundService.remoteConfigController.shouldIgnoreError(
...
extension.runtime.onInstalled.addListener(async (details) => {
  ...
  if (details.reason === extension.runtime.OnInstalledReason.UPDATE) {
    ...
    const storageContents = await new Promise((resolve) => extension.storage.local.get(resolve));
...
const Adapter = getAdapterByType('seed' as any);
const adapter = new (Adapter as any)('test seed for get seed adapter info');
...
async function setupBackgroundService() {
  ...
  // SECURITY: Only expose in development builds. Production builds must have
  // NODE_ENV=production set at build time to ensure this is dead-code-eliminated.
  if (
  ...
  const updateBadge = () => {
    ...
    const notifications =
      backgroundService.notificationsController.getGroupNotificationsByAccount(selectedAccount);
...
class BackgroundService extends EventEmitter {
  ...
  constructor(options: any = {}) {
    ...
    this.networkController.store.subscribe(() => this.currentAccountController.updateBalances());

    this.walletController.store.subscribe(() => this.currentAccountController.updateBalances());
  ...
  getState() {
    ...
    const myNotifications =
      this.notificationsController.getGroupNotificationsByAccount(selectedAccount);
  ...
  getApi() {
    ...
    return {
      ...
      sendEvent: async (event, properties) => this.statisticsController.addEvent(event, properties),
      ...
      signAndPublishTransaction: (data) => newMessage(data, 'transaction', undefined, true),
  ...
  setupUiConnection(connectionStream: any, _processName?: string) {
  ...
  setupPageConnection(connectionStream: any, origin: any) {
    // SECURITY: Early origin validation — reject connections from known-denied origins
    // before establishing the full DNode API surface. This prevents denied origins from
    // even setting up the RPC channel.
    const permission = this.permissionsController.hasPermission(origin, PERMISSIONS.APPROVED);
    if (permission === false) {
      // Origin has been explicitly denied — do not set up any API
      if (connectionStream.end) connectionStream.end();
      return;
    }
